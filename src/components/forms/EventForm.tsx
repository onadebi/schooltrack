...
const EventForm: React.FC<IPropsTableForm<RegInput>> = ({type, data}) => {
...
const {
  ...
  return (
    ...
    <form className="flex flex-col gap-8" onSubmit={OnSubmit}>
        <h1 className='xt-xl font-semibold'>{type === 'create' ? `Register new`: 'Update'} event</h1>
        <span className='text-xs font-semibold'>Event Information</span>
        <div className="flex flex-wrap gap-8">
          <FormInputField label='Title' type='text' register={register} name='title' error={errors.firstName} defaultValue={data?.title} inputProps={{placeholder: 'Title'}}/>
          ...
          <FormInputField label='Date' type='date' register={register} name='date' error={errors.date} defaultValue={data?.date ? data.date.toString() : appsettings.functions.FuturDate()} inputProps={{placeholder: 'Date'}}/>

          <FormInputField label='Start time' type='time' register={register} name='startTime' error={errors.startTime} defaultValue={data?.startTime} inputProps={{placeholder: 'Start time'}}/>

          <FormInputField label='End time' type='time' register={register} name='endTime' error={errors.endTime} defaultValue={data?.endTime} inputProps={{placeholder: 'End time'}}/>
