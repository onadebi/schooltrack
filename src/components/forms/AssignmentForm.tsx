...
const AssignmentForm: React.FC<IPropsTableForm<RegInput>> = ({type, data}) => {
...
const {
  ...
  return (
    ...
    <form className="flex flex-col gap-8" onSubmit={OnSubmit}>
        <h1 className='xt-xl font-semibold'>{type === 'create' ? `Register new`: 'Update'} assignment</h1>
        <span className='text-xs font-semibold'>Assignment Information</span>
        <div className="flex flex-wrap gap-8">
          ...
          <FormInputField label='Due date' type='date' register={register} name='dueDate' error={errors.dueDate} defaultValue={data?.dueDate ? data.dueDate.toString() : new Date().toISOString().split('T')[0]} inputProps={{placeholder: 'Due date'}}/>
