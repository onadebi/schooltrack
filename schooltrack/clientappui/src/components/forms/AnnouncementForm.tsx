...
const AnnouncementForm: React.FC<IPropsTableForm<RegInput>> = ({type, data}) => {
...
const {
  ...
  return (
    ...
    <form className="flex flex-col gap-8" onSubmit={OnSubmit}>
        <h1 className='xt-xl font-semibold'>{type === 'create' ? `New`: 'Update'} announcement</h1>
        <div className="flex flex-wrap gap-8">
          <FormInputField label='Title' type='text' register={register} name='title' error={errors.title} defaultValue={data?.title} inputProps={{placeholder: 'First Name'}}/>
          ...
          <FormInputField label='Date' type='date' register={register} name='date' error={errors.date} defaultValue={data?.date ? data.date.toString() : appsettings.functions.FuturDate()} />
